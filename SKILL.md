...
- **玩法**：从产品真实能力出发（做券→发券→领券→核销→统计哪一段被用上），写成一句"这个行业的老板该怎么做"。**动笔前先查 `facts.forbidden_claims`**——能力边界写错，整条片子作废（例：券包与次卡只支持私密发放）。
...
- **字数与时长不设固定区间（2026-08-25 用户拍板恢复，08-28 曾被我私设成 420-450 字，作废）**：先讲满是硬要求——所选行业用到的功能与关联面讲全、干货足（≥6 信息单元 + 每屏可读 ≥3 条）；时长按复杂度自然落位（简单 60-90s / 标准 90-120s / 复杂 120-150s，硬上限 200s，超了拆上下集）。语速 ≤6 字/秒，冗余句 ≤8%。不为凑长塞话，也不为显短砍干货——检验法：删掉一半会损失重要信息，才算讲满。
