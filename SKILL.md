...
- **质感锚（2026-08-29 用户逐张认可，做屏前先看这四张图）**：`outputs/bench-bundle-open.png`（产品特写）· `bench-pain.png`（痛点·实物传单）· `bench-mech.png`（机制对比）· `bench-table.png`（多列对照表，ref-15 拆底稿实证）；实现代码在 `video/src/bench/`——新屏从锚稿抄结构，禁止脱离自由发挥；对不上锚稿 = 回炉。锚未覆盖的新屏型走「ref 拆底稿 → 一屏标杆」路径（17 屏型底稿样张已就位，`outputs/bench/bench-rNN.png`，待审）。一条片内每屏按叙事功能挑**不同**的锚/底稿——片内布局多样、跨片骨架稳定。
