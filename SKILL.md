...
- **质感锚（2026-08-29 用户逐张认可，做屏前先看这四张图）**：`outputs/bench-bundle-open.png`（产品特写）· `bench-pain.png`（痛点·实物传单）· `bench-mech.png`（机制对比）· `bench-table.png`（多列对照表，ref-15 拆底稿实证）；实现代码在 `video/src/bench/`——新屏从锚稿抄结构，禁止脱离自由发挥；对不上锚稿 = 回炉。锚未覆盖的新屏型走「ref 拆底稿 → 一屏标杆」路径（已验证可行）。
...
- **新屏型先找排版底稿**：三张质感锚覆盖不了的屏型（多列对照表、四象限、清单卡等），从 `outputs/样本库/ref-01~19` 挑最接近的一张，拆它的栅格/字号对比/用色做新组件（只借骨架不抄内容，配色换产品主题色），照旧先过一屏标杆。新参考图按 `ref-NN-屏型名-来源.扩展名` 命名入库即可走同一流程。
