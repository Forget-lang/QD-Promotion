...
const BoardHeading: React.FC<{ text: string; delay?: number; tone?: string }> = ({ text, delay = 6, tone = EMBER_LT }) => (
  <WriteIn start={delay}>
    <div style={{ fontFamily: FONT_BODY, fontFamily: FONT_TITLE, fontWeight: 700, fontSize: 40, color: tone, letterSpacing: 3, textShadow: CHALK_SHADOW }}>{text}</div>
...
const RowLine: React.FC<{ row: FormRow; start: number }> = ({ row, start }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', transform: `translateX(${x}px)`, opacity: o, padding: '18px 0', borderBottom: '1px dashed rgba(198,189,181,.22)' }}>
      ...
      <span style={{ fontFamily: FONT_BODY, fontFamily: FONT_TITLE, fontSize: 44, color: row.hero ? GOLD : CHALK, fontWeight: 700, textShadow: CHALK_SHADOW }}>{row.v}</span>
    </div>
  );
};
  ...
  return (
    <AbsoluteFill>
      ...
      <Board h={1150} top={300} sp={sp}>
        ...
        <WriteIn start={30}>
          <div style={{ fontFamily: FONT_BODY, fontSize: 32, color: CHALK_DIM, marginTop: 16 }}>券类型：{p.couponType}</div>
        </WriteIn>
