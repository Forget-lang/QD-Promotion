...
// 券行数据（真实形态示例值）
const COUPONS = [
  { thumb: '兑换券', name: '45 分钟美术体验课兑换券', amount: null as string | null, unit: '', threshold: '到店出示核销码', valid: '有效期：领取后 30 天内', count: 'x1张' },
  { thumb: '满减券', name: '秋季报名满减券', amount: '80', unit: '元券', threshold: '满 800 元可用', valid: '有效期：领取后 30 天内', count: 'x1张' },
  { thumb: '折扣券', name: '画材专用折扣券', amount: '9', unit: '折券', threshold: '画材店内在售商品', valid: '有效期：领取后 60 天内', count: 'x1张' },
];
...
export const BundleOpenBench: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: '#F6F6F6', fontFamily: FONT_BODY }}>
      ...
      <div style={{
        position: 'absolute', left: 0, top: 0, right: 0, height: 600,
        ...
        {/* 海报标题区 */}
        <div style={{ position: 'absolute', left: 64, top: 224, color: '#fff' }}>
          ...
          <div style={{ display: 'flex', gap: 14, marginTop: 26 }}>
            {['有效期 30 天', '到店出示核销', '每人限领 1 份'].map((t) => (
              <span key={t} style={{ fontSize: 24, color: 'rgba(255,255,255,0.9)', border: '1.5px solid rgba(255,255,255,0.4)', borderRadius: 999, padding: '8px 20px', background: 'rgba(255,255,255,0.07)' }}>{t}</span>
            ))}
          </div>
        </div>
        <StatusBar />
      ...
      <div style={{
        position: 'absolute', left: 32 * PX, right: 32 * PX, top: 680,
        background: THEME, borderRadius: 35, padding: '34px 30px 16px',
        boxShadow: '0 18px 44px rgba(18,52,72,0.28)',
      }}>
        <div style={{ fontSize: 44, fontWeight: 700, color: '#fff', lineHeight: 1.35 }}>开学季三选一 · 专属优惠券</div>
        <div style={{ marginTop: 12, fontSize: 30, color: 'rgba(255,255,255,0.85)' }}>私密发放 · 3张专属优惠券</div>

        {COUPONS.map((c, i) => {
          const p = [card1, card2, card3][i];
          return (
            <div key={c.name} style={{
              position: 'relative', marginTop: 18, background: '#fff', borderRadius: 35, padding: '20px 28px',
              ...
              <div style={{ width: 96, height: 96, borderRadius: 16, background: '#F5F7F9', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ marginTop: 8, fontSize: 27, color: '#B6B6B6' }}>{c.valid}</div>
              </div>
        ...
        {/* 使用须知（真值 .bundle-content 压缩为两行，完整入画） */}
        <div style={{ marginTop: 20, paddingBottom: 10 }}>
          <div style={{ fontSize: 28, fontWeight: 600, color: 'rgba(255,255,255,0.68)' }}>使用须知</div>
          <div style={{ marginTop: 8, fontSize: 25, lineHeight: 1.5, color: 'rgba(255,255,255,0.62)' }}>
            <span style={{ color: 'rgba(255,255,255,0.85)' }}>每人限领 1 个</span> · 体验课需提前与老师约时间 · 到店出示核销码
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        boxShadow: '0 -3px 23px rgba(0,0,0,0.06)', padding: '26px 43px 34px',
        ...
        <div style={{ marginTop: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontSize: 26, color: '#999' }}>
          {/* 倒计时方块（t-count-down square 风格） */}
          {['01', '23', '45'].map((n) => (
            <span key={n} style={{ background: '#2B2B2B', color: '#fff', borderRadius: 5, padding: '2px 7px', fontSize: 22, fontWeight: 500 }}>{n}</span>
          ))}
          <span>后领取链接失效，请尽快领取</span>
        </div>
