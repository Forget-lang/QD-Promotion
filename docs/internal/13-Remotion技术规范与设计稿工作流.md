...
### 3.3 行业背景图（B 方案 · 每行业一张专属）

> 每条视频配一张行业专属背景图作为氛围底层，提升质感和行业代入感。
...
| 参数 | 值 | 原因 |
|---|---|---|
| 模糊 | 15px 高斯模糊 | 消除细节，只保留色彩和光影氛围 |
| 不透明度 | 55% | 图是氛围层，不能压过 UI 组件 |
| 主色统调 | 背景图上叠 palette 主色 `soft-light` 混合 18%（`AccentOverlay` 组件） | 行业照片与 UI 色系融合，不再需要蒙版（2026-08-20） |
| Ken Burns | 背景图全片线性缩放 1.0→1.04（VTemplate 内 interpolate） | 静态照片极慢推近变「活」，仍只做氛围不承担信息（2026-08-20） |
| 叠加层 | 场景背景色半透明（深色 70% / 浅色 75% / CTA 85%） | 纯色叠加保证文字对比度，不同场景透明度不同 |
| 覆盖范围 | 全视频所有屏共用同一张 | 形成系列感，减少素材量 |

#### 全片氛围层（VTemplate 统一挂载，场景与设计稿无需处理）

> 2026-08-20 起全片自动叠加两层质感，参数为视频级常量，**不逐屏调、不写进设计稿**：

| 层 | 组件 | 参数 | 作用 |
|---|---|---|---|
| 暗角 | `Vignette` | 径向渐变，55% 中心透明 → 四角 rgba(0,0,0,0.25) | 聚焦中部视线，压住模糊背景图边缘发灰 |
| 颗粒 | `Grain` | feTurbulence 噪点 240px 平铺，opacity 3.5%，静态不动画 | 杀纯色渐变的色带（banding），全片图层质感统一 |

#### 选图标准

✅ 要：
- 与行业强相关（咖啡→咖啡杯/店内景，美业→护肤品/沙龙）
- 暖色调或与行业主色协调
- 画面简洁，留白多
- 氛围感强（虚化好、光影美）
...
- 人选取后复制到：`video/public/backgrounds/gXX/bg.jpg`
...
<AbsoluteFill>
  <Img src={staticFile('backgrounds/g02/g02-s1-bg.jpg')}
       style={{width:'100%',height:'100%',objectFit:'cover',filter:'blur(15px)',opacity:0.55}} />
  <AbsoluteFill style={{background:'rgba(22,24,42,0.7)'}} />
  {/* 文字在上层 */}
</AbsoluteFill>
...
<AbsoluteFill>
  <Img src={staticFile('backgrounds/g03/g03-s1-bg.jpg')} style={{width:'100%',height:'100%',objectFit:'cover',filter:'blur(15px)',opacity:0.55}} />
  <AbsoluteFill style={{background:'rgba(42,26,18,0.7)'}} />
  {/* 文字内容在上层 */}
</AbsoluteFill>
...
背景图: IMG-G02-C01（咖啡店场景，模糊处理）
  ...
  - 处理方式: 高斯模糊 15px + 透明度 55% + 场景背景色半透明叠加
