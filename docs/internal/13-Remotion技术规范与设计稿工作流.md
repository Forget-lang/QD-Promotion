...
| **字幕安全区** | 有字幕的视频，字幕放在 y:1760-1920 底部区域内（距底 80px 居中），不侵入内容区；画面主体内容全部在 y:120-1760 活动区内 |
...
### 2.6 J-cut 转场同步法（v4 标准方案 · 2026-08-19 定稿）
...
- 最后 `TRANSITION_FRAMES`（12帧=0.4s）音量从 1 线性降到 0
- 下一屏音频在转场第 0 帧以满音量进入
- 最后一屏（CTA）最后 0.4s 也淡出到零，自然收尾
...
> 2026-08-19 确定：每条视频配一张行业专属背景图作为氛围底层，提升质感和行业代入感。
...
| 参数 | 值 | 原因 |
|---|---|---|
| 模糊 | 30px 高斯模糊 | 消除细节，只保留色彩和光影氛围 |
| 不透明度 | 30% | 图是氛围层，不能压过 UI 组件 |
| 蒙版 | 主色线性渐变（顶部深 → 底部稍浅） | 统一色调，保证文字对比度 |
...
以下元素是品牌「神」，每条视频保持一致：
- 品牌署名「券到卡包」（CTA 屏 title，得意黑，白色；副标题「再小的门店也可以制作电子券」）
...
**+ 叙事结构差异（2026-08-18 补充）**：除视觉维度外，还须检查场景类型序列（即各屏的 `type` 顺序）是否与已有行业视频不同。不同行业必须使用**不同的场景序列**——至少 3 个位置上的场景类型与已有视频不同。禁止"换个行业词、结构照搬"的做法。新行业首做时，须先扫描已有视频的序列，确保本片序列不重复。

### 5.4 分镜表（逐屏填写）

**内容深度铁律（2026-08-18 决策，禁止"功能罗列"）**：
- 每条视频只讲 **≤2 个核心功能**，宁深勿浅。禁止一条视频覆盖 4-5 个功能（如次卡+转赠+券种搭配+核销奖励+到期提醒），每个功能只提一嘴即过。
- 选定 1-2 个功能后，**所有场景围绕这个核心展开**：钩子引出痛点，痛点自然引出该功能，解法屏深入讲透它，后续屏展示它如何运作、带来什么效果，CTA 收束。场景之间要有**叙事递进**，而非"换一个功能讲"。
- 判断标准：如果某屏的主题与上一屏无关（换了一个产品功能），说明在功能罗列，须删减。
- 反面例子：S3 讲次卡三特性，S4 讲转赠流程，S5 讲券种搭配，S6 讲数据面板——四屏四个独立功能，每个都浅。正确做法：只讲次卡，S3 讲次卡是什么，S4 讲次卡如何锁客，S5 展示次卡带来的效果，S6 收尾。
...
时长: 5s（150帧）                  # 设计稿定义固定 dur，口播稿文字量按 §5.6 校准
标题: （主标题文字，≤14字）
副标题: （辅助文字，≤30字）
...
**J-cut 标准流程（2026-08-19 定稿，替代旧的 dur 校准规则）：**

```
1. 设计稿写口播文案（按 4-5字/秒 估算大致节奏）
...
> **设计依据**：09 最佳实践库 §6.2「封面设计与留存优化」+ 本节版式库
> **AI 职责**：按本规范产出封面设计稿（版式选型 + 文案 + 找图规格），人确认后由设计平台（创可贴/Canva）或 AI 生图工具制作
...
**三平台封面规则**：
- 抖音 + 小红书：**共用同一张竖版封面**（1080×1920），主视觉一致
- 搜狐长文：**单独做横版头图**（建议 1200×675，16:9），风格统一但排版重新适配
...
> **设计稿是最终权威**（2026-08-18 决策）：设计稿定义每屏的固定 dur 和口播稿文字量。口播稿按 §5.6 规则校准文字量，TTS 自然语速生成后自动匹配，无需任何一方驱动另一方。禁止 atempo 硬拉语速。
...
- ffprobe 测每段时长 → 按 §2.6 公式算 dur → 写入 `data/gXX.ts`
- 直接渲染成片（无需先出样片确认语速，自然语速 + J-cut = 一次到位）
- 验证：听最终 MP4，检查首字清晰度、转场爆音、整体节奏（§2.6 验证清单）

**⑧ 成片渲染**：`npx remotion render`（含音频），前置 `NODE_OPTIONS=""`。整片时长 = 设计稿总时长，声画帧级同步。
...
| `cta` | CtaScene | title, sub | 全屏accent色+白色GlowOrb柔光，品牌署名+装饰线draw-on+标题微脉冲；title=「券到卡包」，sub=「再小的门店也可以制作电子券」 |
...
> **完整铁律红线唯一真源：`quandao-content` skill §二**。本节仅列视频代码层高频检查项，详细规则以 SKILL 为准。
>
