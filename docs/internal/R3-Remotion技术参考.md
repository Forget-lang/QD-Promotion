...
**使用规则**：
1. 设计稿每屏标注「呈现手法」字段（7 选 1：编号列表 / 手写高亮 / 多层分区 / 撕纸笔记 / 括号分组 / 大边框分区 / 药丸徽章；卡片容器不算手法——它是默认承载形式，全片只用卡片=「卡片流」，正是本节要防的）
