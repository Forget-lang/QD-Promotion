...
## 十三、范围外订正（2026-09-17 12:2x，由 `CHANGE-20260917-011` 发起，本事务正文不回填）

> 本节只登记"本事务的哪条断言已被后续事实推翻"，不修改上文任何原段落，避免回填历史契约。

1. **§七 Impact Map 中 `outputs/archive/changelog-2026-08.md` 那一行的 `PRESERVE / DONE`（"全文保留，除头部 1 行"）已被推翻**：`d896110`（2026-09-17 12:14「删除changelog」）把该分卷**整份删除**（`outputs/archive/changelog-2026-08.md`，2474 行 / 132 条）。该删除未走 R8、未建契约，且**未触发 CI**（该文件不在任何工作流 `paths` 内）。
2. **连带失效**：§十二「已保留」一条同样凭的是上述断言；§十二「旧口径扫描」中"仍在用的 `changelog-2026-08.md`"这一前提已不成立。
3. **归属**：分卷退役本身不在本事务范围内，由 `CHANGE-20260917-011`（历史账本全系退役与悬空指针清零）负责收口——清 8 处引用、扩展闸门退役名拦阻至两个名字、收窄历史轮次叙事的豁免白名单。
4. **本事务关闭前须做**：以 011 的最终状态为准复核本事务的 §十二 证据（尤其"本次新增红 0"与"5 处硬失败＝基线"两项），不得直接沿用上文的数值。恢复该分卷的命令：`git checkout d03e606 -- outputs/archive/changelog-2026-08.md`。
