 ...
 *
 * 为什么要有它：G04/G05 曾靠 AI 自评四维打勾通过（自评 4~4.5 分），实测却有 82% 时间画面几乎静止。
 * 能算的指标就不要留给自觉。
 *
 * 用法：
 *   node scripts/check-motion.mjs outputs/g04-美容院/g04-成片-v2.mp4      # 整片运动指标
 *   node scripts/check-motion.mjs outputs/g05-兴趣班/g05-s1.png ...       # 静态图只算画面占用率
 *   node scripts/check-motion.mjs <mp4> --json                            # 机器可读输出
 *
 * 指标与合格线（SKILL 第 6 步）：静止占比 ≤35%｜最长静止段 ≤3s｜平均帧间差 ≥2.2｜画面占用率 ≥70%
 * 参照：我们 G04 = 82% / 14s / 1.52；同事参考片 = 30% / 6s / 2.65
 */
...
/**
 * 阈值怎么来的（2026-08-28 校准）：原先用绝对阈值 1.2，结果同一参考片在 32×32 与 64×64 两种采样下
 * 算出 30% 与 77% —— 绝对阈值随采样分辨率漂移，不可信。改用两片实测分布定标：
 *   中位帧间差：G04（观感差）= 0.13｜同事参考片（观感好）= 0.46
 *   → 死帧线取 0.3（落在两者之间偏 G04 一侧，能稳定区分"没动"与"微动"）
 *   → 合格线：静止占比 ≤45%、中位帧间差 ≥0.35、画面占用率 ≥70%
