...
| 独立静态宣传贴图策划与视觉设计 | `docs/internal/贴图宣传作业规范.md` | 不依附 `gXX` 行业视频的产品介绍/经营攻略静态贴图的方法 Owner；不负责既有抖音图文伴生线、最终发布 |
...
| 风格层（表达能力包） | 视频风格包＝`video/styles/`；独立静态贴图风格包＝`poster/styles/`；视频清单/闸门声明源＝`scripts/ref-registry.json` 的 `styles`，贴图风格声明源＝同表 `posterStyles` | 用什么视觉语言表达；不拥有内容流程、叙事决策与观看路径。视频风格包服务视频内容线；贴图风格包服务独立静态贴图；两者**物理隔离、登记隔离、闸门隔离**，不得把贴图 PNG 资产接入视频 `styles` 的片认领/轮换体系 |
...
当前项目存在两条视频内容线、一项**独立静态贴图的方法层**，以及与其配套的**独立静态贴图风格层**：

- 行业场景视频 → `SKILL.md`
- 产品功能教程 → `docs/internal/产品功能教程作业规范.md`
- 独立静态宣传贴图的方法 → `docs/internal/贴图宣传作业规范.md`

### 静态贴图与既有抖音图文线的判线

- **随 `gXX` 行业视频伴生的抖音图文**：仍由 `SKILL.md` 独占生产 Owner，继续沿用既有 1~5 张、图文数据文件、发布稿、后验与工程路径。
- **不依附行业视频的独立产品介绍/经营攻略贴图**：由 `docs/internal/贴图宣传作业规范.md` 提供唯一的方法与视觉设计路径；它不是视频 `contentLines`，不创建第二套抖音图文生产线，也不接管既有图文工程/后验账。

判线首先看**是否伴生 `gXX` 行业视频**，而不是看文件是 PNG 还是任务里是否出现“图文”两个字。

两类静态资产可以共享 Product Truth、内容资源边界、平台红线和视觉经验，但不得复制另一类的生产流程或 Owner 规则。
...
行业线的「画面从哪来」由**视频风格层**承担：每片先按 `styles` 声明选定风格包并声明 `styleId`；风格包**不构成入口**，经 `SKILL.md`／`R9` 内部路由（清单与逐闸门适用性只认 `scripts/ref-registry.json` 的 `styles`）。

独立静态宣传贴图的「怎么画」由**贴图风格层**承担：方法 Owner 负责选题／内容／视觉意图并选定 `styleId`；具体字体、色彩、材质、版式、插画/3D表达与静态画布规则由 `poster/styles/<styleId>/SKILL.md` 承担，声明源为 `scripts/ref-registry.json` 的 `posterStyles`。贴图风格包**不构成生产入口**，不进入视频 `styles` 的片认领、风格轮换或视频闸门体系。
...
- 风格轮换与跨风格验收（行业线）：**同一风格不得连续复用**；跨风格验收＝**导演稿替换测试**——同一份《导演稿》换任一风格包，都不得要求改写叙事结构／信息释放顺序／观看路径／情绪曲线（判据在 `R9` §十五，长期有效；越界即该风格包问题）。独立静态贴图暂不接入视频风格轮换机检；贴图之间只在**同一贴图风格包内部**做视觉隔离验收，未来若建立贴图轮换/相似度机检，另立事务，不把视频规则直接外溢。
