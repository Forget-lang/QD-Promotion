...
> 最后校验：2026-09-23。任何 AI 助手在本仓库开工前先读本文件。
...
| 风格层（表达能力包） | `video/styles/` 下每风格一个自包含目录（清单与逐闸门适用性声明源＝`scripts/ref-registry.json` 的 `styles`） | 用什么视觉语言表达（镜头表现方式／空间／素材／动效／声音）；不拥有内容流程、叙事决策与观看路径 |
